...
Each library will contain a few security definitions that implement the specific encryption
...
program has a significant advantage but occasionally fails, so running many
...
methods that are defined for that security definition, for example one time
...
be implemented for each security definition:
...
This function will store however many shares specified by the scheme into U.
...
This function returns a pseudorandom number based on the implementation.

###PseudoRandom Function (Prf):

#### LOOKUP(x):

This function returns a pseudorandom number generated from the function
...
This function returns a psuedorandom number generated from the permutation
function specified by the scheme and is invertible by INVERSE(). For
example, INVERSE(LOOKUP(x)) == x.

#### INVERSE(y):

This function returns a psuedorandom number generated from the permutation
function specified by the scheme and is invertible by LOOKUP(). For
